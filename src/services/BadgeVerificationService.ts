...
// Import type guards from openbadges-types
import * as OB2Guards from 'openbadges-types/dist/v2/guards';
import * as OB3Guards from 'openbadges-types/dist/v3/guards';
...
export class BadgeVerificationService {
   ...
   * @returns Verification result
   */
  private static async verifyOB2Badge(badge: OB2.Assertion, result: VerificationResult): Promise<VerificationResult> {
    // Set badge version
    ...
    if (!badge.verification) {
      ...
      if (!verificationType) {
        ...
        if (normalizedType === 'hosted') {
          ...
          result.contentValidation.errors.push(`Unsupported verification type: ${verificationType}`);
          result.contentValidation.isValid = false;
        }
      }
    }
    ...
    result.isValid = result.structureValidation?.isValid === true &&
    ...
    if (typeof badge.badge === 'object') {
      if (!OB2Guards.isBadgeClass(badge.badge)) {
        ...
        if (!badge.badge.issuer) {
          ...
          result.contentValidation!.isValid = false;
        } else if (typeof badge.badge.issuer === 'object' && !OB2Guards.isProfile(badge.badge.issuer)) {
   ...
   * @returns Verification result
   */
  private static async verifyOB3Badge(badge: OB3.VerifiableCredential, result: VerificationResult): Promise<VerificationResult> {
    // Set badge version
    ...
    if (!badge.proof) {
      ...
      if (!proofType) {
        ...
        result.contentValidation.warnings.push('Full cryptographic verification of OB3 credentials is not yet implemented');
    ...
    if (badge.credentialStatus) {
      if (!OB3Guards.isCredentialStatus(badge.credentialStatus)) {
        result.contentValidation.errors.push('OB3 credential has an invalid credential status structure');
        result.contentValidation.isValid = false;
      } else {
        ...
        result.contentValidation.warnings.push('Credential status checking requires network access to validate against the status endpoint');
      }
    } else {
   ...
   * @param result The verification result to update
   */
  private static validateOB3Components(badge: OB3.VerifiableCredential, result: VerificationResult): void {
    // Validate context
    if (!badge['@context'] || !Array.isArray(badge['@context'])) {
      ...
      const hasW3CContext = badge['@context'].some(ctx =>
        ctx === 'https://www.w3.org/2018/credentials/v1' ||
        ctx === 'https://w3id.org/credentials/v1');

      const hasOB3Context = badge['@context'].some(ctx =>
        ctx === 'https://purl.imsglobal.org/spec/ob/v3p0/context.json' ||
        ctx === 'https://w3id.org/openbadges/v3');

      if (!hasW3CContext) {
        result.contentValidation!.errors.push('OB3 credential is missing the W3C Verifiable Credentials context');
        result.contentValidation!.isValid = false;
      }

      if (!hasOB3Context) {
        result.contentValidation!.errors.push('OB3 credential is missing the Open Badges v3 context');
        result.contentValidation!.isValid = false;
      }
    }
    ...
    if (!badge.type || !Array.isArray(badge.type)) {
      ...
      if (!hasVCType) {
        result.contentValidation!.errors.push('OB3 credential is missing the VerifiableCredential type');
        result.contentValidation!.isValid = false;
      }

      if (!hasOBType) {
        result.contentValidation!.errors.push('OB3 credential is missing the OpenBadgeCredential type');
        result.contentValidation!.isValid = false;
      }
    }
    ...
    if (!badge.credentialSubject) {
      ...
      result.contentValidation!.errors.push('OB3 credential has an invalid credential subject structure');
      result.contentValidation!.isValid = false;
    } else {
      ...
      if (!badge.credentialSubject.achievement) {
        ...
        result.contentValidation!.errors.push('Credential subject has an invalid achievement structure');
        result.contentValidation!.isValid = false;
      } else {
        ...
        if (badge.credentialSubject.achievement.criteria &&
        ...
        if (badge.credentialSubject.achievement.image) {
          if (typeof badge.credentialSubject.achievement.image === 'object' &&
    ...
    if (badge.refreshService && !OB3Guards.isRefreshService(badge.refreshService)) {
      result.contentValidation!.errors.push('OB3 credential has an invalid refresh service structure');
      result.contentValidation!.isValid = false;
    }
      ...
      if (Array.isArray(badge.termsOfUse)) {
        for (const terms of badge.termsOfUse) {
          if (!OB3Guards.isTermsOfUse(terms)) {
            result.contentValidation!.errors.push('OB3 credential has an invalid terms of use structure');
            result.contentValidation!.isValid = false;
            break;
          }
        }
  ...
  private static async verifyHostedBadge(badge: OB2.Assertion, result: VerificationResult): Promise<void> {
    ...
    result.contentValidation!.warnings.push('Hosted badge verification requires network access to validate against the hosted assertion');
  ...
  private static async verifySignedBadge(badge: OB2.Assertion, result: VerificationResult): Promise<void> {
    ...
    if (!verification.creator && typeof badge.badge === 'object' && badge.badge.issuer) {
      ...
      result.contentValidation!.warnings.push('Signed badge does not specify which key was used for signing');
    }
    ...
    result.contentValidation!.warnings.push('Full signature verification requires cryptographic libraries');
  }
  ...
  private static async checkRevocation(badge: OB2.Assertion | OB3.VerifiableCredential, result: VerificationResult): Promise<void> {
    ...
    if (isOB2Assertion(badge)) {
      if (badge.revoked === true) {
        ...
        result.contentValidation!.errors.push(badge.revocationReason
    ...
    if (badge.credentialStatus) {
      ...
      result.revocationStatus = 'unknown';
      result.contentValidation!.warnings.push('Revocation checking for OB3 credentials is not fully implemented');
    }
  }
