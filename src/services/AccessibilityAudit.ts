...
export class AccessibilityAudit {
  ...
  static checkCognitiveAccessibility(element: HTMLElement): Array<{
    ...
    paragraphs.forEach(p => {
      ...
      sentences.forEach(sentence => {
        ...
        if (sentenceWords.length > 25) {
          issues.push({
            severity: 'warning',
            message: 'Sentence is too long (more than 25 words). Consider breaking it up for better readability.',
      ...
      if (complexWords.length > words.length * 0.1) {
        issues.push({
          severity: 'info',
          message: 'Text contains many complex words. Consider simplifying language for better cognitive accessibility.',
          element: p as HTMLElement
        });
      }
    });
    ...
    if (headings.length === 0 && element.textContent && element.textContent.length > 200) {
      issues.push({
        severity: 'warning',
        message: 'Long content without headings. Consider adding headings to improve structure and readability.',
        element: element
      });
    }
    ...
    navElements.forEach(nav => {
      ...
      if (links.length > 7) {
        issues.push({
          severity: 'info',
          message: 'Navigation contains many items. Consider grouping or simplifying navigation for users with cognitive disabilities.',
    ...
    forms.forEach(form => {
      ...
      if (instructions.length === 0 && form.querySelectorAll('input, select, textarea').length > 0) {
        issues.push({
          severity: 'info',
          message: 'Form lacks clear instructions. Consider adding help text for users with cognitive disabilities.',
  ...
  static checkSensoryAccessibility(element: HTMLElement): Array<{
    ...
    const animatedElements = element.querySelectorAll('[class*="animate"], [class*="motion"], [class*="transition"]');
    animatedElements.forEach(el => {
      const style = window.getComputedStyle(el);
      if (style.animation !== 'none' || style.transition !== 'none') {
        issues.push({
          severity: 'info',
          message: 'Element has animations or transitions. Ensure users can disable animations for vestibular disorders.',
          element: el as HTMLElement
        });
      }
    });
    ...
    const contentAreas = element.querySelectorAll('section, article, div[class*="content"], div[class*="container"]');
    contentAreas.forEach(area => {
      ...
      if (colors.size > 7) { // More than 7 different colors might be overwhelming
        issues.push({
          severity: 'info',
          message: 'Content area uses many different colors. Consider simplifying the color palette for users with sensory sensitivities.',
      ...
      if (density > 1 && childElements > 10) { // Arbitrary threshold
        issues.push({
          severity: 'info',
          message: 'Content area appears densely packed. Consider adding more white space for users with sensory processing issues.',
          element: area as HTMLElement
        });
      }
    });
    ...
    const potentiallyFlashingElements = element.querySelectorAll('[class*="flash"], [class*="blink"], [class*="pulse"]');
    if (potentiallyFlashingElements.length > 0) {
      issues.push({
        severity: 'warning',
        message: 'Potentially flashing or blinking content detected. Ensure content doesn't flash more than 3 times per second to avoid seizure risks.',
