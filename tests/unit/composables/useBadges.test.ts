...
describe('useBadges', () => {
  it('initial state is empty', () => {
    const { badges, filterText, sortBy, sortDirection, filteredBadges, sortedBadges, normalizedBadges, badgesByIssuer } = useBadges();
  ...
  it('setBadges replaces badges', () => {
    ...
    const a = BadgeService.createAssertionTemplate(BadgeService.createBadgeClassTemplate(), 'a@a.com');
    const b = BadgeService.createAssertionTemplate(BadgeService.createBadgeClassTemplate(), 'b@b.com');
    setBadges([a, b]);
  ...
  it('filterText and filtering works', () => {
    ...
    const badge1 = BadgeService.createAssertionTemplate(BadgeService.createBadgeClassTemplate(), 'e1@e.com');
    (badge1.badge as OB2.BadgeClass).name = 'Alpha Badge';
    const badge2 = BadgeService.createAssertionTemplate(BadgeService.createBadgeClassTemplate(), 'e2@e.com');
  ...
  it('sorting by name asc/desc works', () => {
    ...
    const bc1 = BadgeService.createBadgeClassTemplate(); bc1.name = 'A';
    const bc2 = BadgeService.createBadgeClassTemplate(); bc2.name = 'B';
